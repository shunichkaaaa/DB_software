...
<p>Проблема полягає в тому, що дані, які передаються через HTTP (без літери "s" в кінці), не зашифровані і можуть бути перехоплені третіми сторонами для збирання даних, які передаються між двома системами. Це можна вирішити за допомогою безпечної версії, яка називається HTTPS, де "S" означає "безпечний".</p>
<p>Для цього використовується <strong>SSL-сертифікат</strong> - "SSL" означає захищений протокол передачі даних - який створює безпечне зашифроване з'єднання між веб-сервером і веб-браузером.</p>
<p>Без HTTPS будь-які передані дані не є безпечними. Це особливо важливо для сайтів, де через з'єднання передаються важливі дані, такі як сайти електронної комерції, які приймають онлайн-платежі карткою, або вікна входу, де користувачам потрібно вводити свої облікові дані.</p>
