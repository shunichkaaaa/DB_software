...
<div class="section" id="section3">
  <h3>3. HTTP запити 📤</h3>
	<p>HTTP - протокол передачі гіпертексту. Це протокол, що дозволяє комунікацію між різними системами. Найчастіше він використовується для передачі даних з веб-сервера до браузера для перегляду веб-сторінок. HTTP визначає набір методів запиту, які вказують на бажану дію для вказаного ресурсу. Хоча вони також можуть бути іменниками, ці методи запиту іноді називаються HTTP-дієсловами. Кожен з них реалізує різні семантики, але деякі загальні риси спільні для групи з них, наприклад, метод запиту може бути безпечним, ідемпотентним або кешованим.</p>
	<dl>
  ...
  <dt id="connect"><code>CONNECT</code></dt>
  <dd>
  ...
  <dt id="patch"><code>PATCH</code></dt>
  <dd>
...
<p><strong>Що таке HTTPS і чим він відрізняється від HTTP?</strong></p>
<p style="text-align:justify">Проблема полягає в тому, що дані, які передаються через HTTP (без літери "s" в кінці), не зашифровані і можуть бути перехоплені третіми сторонами для збирання даних, які передаються між двома системами. Це можна вирішити за допомогою безпечної версії, яка називається HTTPS, де "S" означає "безпечний".</p>
<p style="text-align:justify">Для цього використовується <strong>SSL-сертифікат</strong> - "SSL" означає захищений протокол передачі даних - який створює безпечне зашифроване з'єднання між веб-сервером і веб-браузером.</p>
<p style="text-align:justify">Без HTTPS будь-які передані дані не є безпечними. Це особливо важливо для сайтів, де через з'єднання передаються важливі дані, наприклад комерційні сайти, які приймають онлайн-платежі карткою, або вікна входу, де користувачам потрібно вводити свої облікові дані.</p>


<!-- Розділ 4 -->
<div class="section" id="section4">
  <h3>4. Структура типових API та Microservice додатків 🛠</h3>
  <img src="https://loopback.io/pages/en/lb4/imgs/key-concepts-overview-diagram.png"/>
  <li><code>Application</code>: центральний клас для налаштування всіх компонентів, контролерів, серверів і прив’язок вашого модуля. Клас Application розширює контекст і надає засоби керування для запуску та зупинки пов’язаних з ним серверів.</li>
  <li><code>Server</code>: реалізація вхідних транспортів/протоколів, таких як REST (http, https), gRPC (http2) і graphQL (http, https). Зазвичай він прослуховує певну кінцеву точку (протокол/хост/порт), обробляє вхідні запити, а потім повертає відповідні відповіді.</li>
  <li><code>Controller</code>: клас, який реалізує операції, визначені REST API програми. Він реалізує бізнес-логіку програми та діє як міст між API HTTP/REST і моделями домену/бази даних. Контролер працює лише з обробленими вхідними даними та абстракціями внутрішніх служб/баз даних.</li>
  <li><code>Interceptor</code>: функція, яка перехоплює статичні виклики методів або методів екземплярів у класі чи об’єкті.</li>
  <li><code>Route</code>: зіставлення між специфікацією API та операцією. Він повідомляє LoopBack, яку операцію invoke() при отриманні запиту HTTP.</li>
  <li><code>Sequence</code>: групування дій без стану, які контролюють, як сервер відповідає на запити.</li>
  <li><code>Model</code>: визначення об’єкта щодо DataSource Juggler. Модуль @loopback/repository надає спеціальні декоратори для додавання метаданих до класів TypeScript/JavaScript, щоб використовувати їх із DataSource Juggler. Крім того, модуль @loopback/repository-json-schema використовує метадані декораторів для створення відповідної схеми JSON.</li>
  <li><code>DataSource</code>: групування дій без стану, які контролюють, як сервер відповідає на запити.</li>
  <li><code>Repository</code>: пакет послуг, який представляє колекцію даних у DataSource.</li>
  <li><code>Relation</code>: зіставлення між двома моделями, яке описує реальний зв’язок між ними та відкриває CRUD API на основі конфігурації.</li>
  <li><code>Relation</code>: Шаблон, який використовується для приміток або модифікації ваших оголошень класів та їхніх членів метаданими. Ось інфраструктури, завдяки яким усі артефакти працюють разом:</li>
        <li><code>Context</code>: Абстракція станів і залежностей у вашій програмі, яку LoopBack використовує для керування всім. Це глобальний реєстр для всього у вашій програмі (конфігурації, стан, залежності, класи тощо).</li>
  	<li><code>Binding</code>: абстракція елементів, керованих контекстом. Кожне прив’язування має унікальний ключ у контексті та постачальника значення для визначення ключа до значення.</li>
  	<li><code>Dependency Injection</code>: Техніка, яка використовується для відокремлення побудови залежностей класу або функції від його поведінки, щоб зберегти слабкий зв’язок коду.</li>
	<li><code>Component</code>: Пакет, який містить одне або кілька розширень LoopBack.</li>
<!-- Розділ 5 -->
